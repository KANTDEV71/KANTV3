...
/*👆API KEY EX TEAM👆 adquira a sua key com*/o = 'wa.me/+5571982507697'
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Italu🧙‍♂️\n' 
            + 'ORG:Dono do kant;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=557182507697:+55 (71) 98250-7697\n' 
            + 'END:VCARD'
...
tiringa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `O integrante @${num.split('@')[0]} saiu do grupo... VAI PELAS SOMBRAS FUCK👋`
    ...
    kant.on('CB:Blocklist', json => {
      ...
      kant.on('chat-update', async (mek) => {
		try {
			...
			const botNumber = kant.user.jid
			const ownerNumber = [`557182507697@s.whatsapp.net`]
			const ownerNumberB = []
			...
			let pushname = kant.contacts[sender] != undefined ? kant.contacts[sender].vname || kant.contacts[sender].notify: undefined
...
//kant.sendMessage(from, lvup, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}}
}
} catch (err) {
console.error(err)
}
...
kant.sendMessage('5571982507697@s.whatsapp.net', options, text, {quoted: mek})
...
tiringa.sendMessage(from, `offline no momento kant dev mexendo nisso` , text, {quoted: mek})
break
...
case 'kant':
case 'kant':
await kant.sendMessage(from, {displayname: "vitor🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('KANT-BOT', 'KANT')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
...
case 'clearall':
if (!isOwner) return reply('Só o kant pode fazer isso')
...
if (vit == "derrota" ) {
var tes = "A vitória é do kant-BOT"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`kant-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
